...
`cli/stage.ts` filled the form; only `cli/submit.ts` submits, and only what the user named in this run.
...
`blocked_on` (`cli/submit.ts review`). Keep it to that table; the user reads the filled forms themselves
...
application for approval; never resubmit silently. Then `cli/submit.ts record`, quoting a confirmation
you have seen in a snapshot.

**On a Lever posting, the user clicks.** hCaptcha gates the submit button. Hand the open tab over;
it is still open when they come back, so confirm from the page rather than filling it again.

## Recording what comes back

Only what the user reports. Do nothing for a role that is merely quiet.

`cli/submit.ts rejected` marks the posting and deletes its resume file. **When they report an interview

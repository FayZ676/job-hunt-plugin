...
export const fields = () => SECTIONS.flatMap((section) => columns(section).map((column) => `${section}.${column}`));
...
export function set(field: string, value: string) {
  ...
  try {
    db().prepare(`UPDATE ${section} SET ${column}=? WHERE id=1`).run(value.trim());
  } catch {
    throw new Error(`'${value}' is not an answer to ${field} — it takes ${takes(section, column)}`);
  }
}
...
export const answers = () => rows(VIEWS.answers, "SELECT section, field, value FROM answers WHERE value IS NOT NULL");

export const missing = () => rows(VIEWS.unanswered, "SELECT section, field FROM unanswered");

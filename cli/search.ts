...
const program = new Command("job-search").description(
...
program
  ...
  .action(guard(async (terms: string[], options) => {
    ...
    if (options.file)
      return report(replay(database, JSON.parse(fs.readFileSync(options.file, "utf8"))));
    if (!terms.length)
      fail("name what to search for, short and literal; `job-score instructions` says what");

    report(await search(database, {
      terms,
...
program
  ...
  .action(guard((options) => {
    const ruled = rule(open(options.db), {
    ...
    if (!ruled.examined)
      return console.log("nothing pending in postings — search first, or pass --redo");

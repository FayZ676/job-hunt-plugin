...
| Purpose | Path |
| ------- | ---- |
| Only content source | `career/index.md` |
| Output directory | `career/resumes/` |
| Moved here once submitted | `career/resumes/submitted/` |
| Section order | `references/resume-template.md` |
| Spec format | `references/resume-spec-schema.md` |
| Recurring defects to avoid | `career/resume-patterns.md` |

`career/index.md` is the only source. Its `## Profile` section supplies the header block and
...
8. Report gaps: JD requirements with no evidence in `career/index.md`; bullets a missing number would
...
- **One idea per bullet, one sentence, under ~30 words.** `career/index.md` packs three ideas into a
...
- **Real numbers only, from `career/index.md`.** Never invent, round up, or extrapolate. Empty
...
- **Match the verb to the work's real status.** "Shipped", "automated" and "replaced" assert that an
  ...
  project's status line in `career/index.md` before choosing. A projected number is not a number —
  never print one.
...
**`career/index.md` is the only source of truth for a number, a title, or a date.** An older resume,
a LinkedIn profile, or a previously generated bullet is not evidence — those are exactly where
inflated figures survive.

Keep a `## Facts` section at the bottom of `career/index.md` for corrections that keep resurfacing:
...
- **A number goes on the resume only if it appears in `career/index.md`.** No estimating, no
...
always another real bullet in `career/index.md`, another Personal Projects line, or the Publications

...
export const experience = () =>
  one(
    VIEWS.experience,
    "SELECT clock_starts, years, relevant_years FROM experience",
  )!;

export const answers = () =>
  rows(VIEWS.answers, "SELECT section, field, value FROM answers");
...
export function career(): Employer[] {
  ...
  const projects = listing("projects", "ORDER BY seq IS NULL, seq, rowid").map(
    (project) => ({
      ...project,
  ...
  return listing("employers", "ORDER BY seq IS NULL, seq, rowid").map(
    (employer) => ({
      ...employer,
      projects: projects.filter(
        (project) => project.employer_id === employer.rowid,
      ),
    }),
  );
}
...
export function prospect(key: string): Prospect | null {
  const posting = one(VIEWS.prospects, "SELECT * FROM prospects WHERE key=?", [
    key,
  ]);
  ...
  return {
    ...
    events: rows(
      EVENT,
      "SELECT at, status, note FROM events WHERE key=? ORDER BY id",
      [key],
    ),
    staged: one(
      STAGED,
      "SELECT url, screenshot, status, blocked_on FROM staged WHERE key=?",
      [key],
    ),
    fields: rows(
      ANSWER,
      "SELECT label, value, tier, flag FROM staged_fields WHERE key=? ORDER BY rowid",
      [key],
    ),
    aliases: rows(
      z.object({ key: z.string() }),
      "SELECT key FROM postings WHERE canonical_key=?",
      [key],
    ).map((row) => row.key),
  };
}

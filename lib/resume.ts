...
export function build(
  ...
  options: { density: string; keepTyp?: boolean; key?: string; database?: Database },
): Built {
  ...
  if (!options.key || !options.database) return { out, density, recorded: null };

  const row = options.database
    .prepare("SELECT key, status FROM prospects WHERE key=?")
    .get(options.key) as { status: string } | undefined;
  if (!row)
    throw new Error(`no prospect '${options.key}' — the PDF is at ${out}, unrecorded`);
  options.database.prepare("UPDATE postings SET resume=? WHERE key=?").run(out, options.key);
